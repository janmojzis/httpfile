...
int cdb_read(struct cdb *c,char *buf,long long xlen,uint32 pos)
{

  long long r;

  if (c->map) {
    if ((pos > c->size) || (c->size - pos < xlen)) goto FORMAT;
    byte_copy(buf,xlen,c->map + pos);
  }
  else {
    /* if (seek_set(c->fd,pos) == -1) return -1; */
    if (lseek(c->fd,pos,0) == -1) return -1;
    while (xlen > 0) {
      r = xlen;
...
static int match(struct cdb *c,const char *key,long long len,uint32 pos)
{
  char buf[32];
  long long n;
...
int cdb_findnext(struct cdb *c,const char *key,long long len)
{
  char buf[8];
  uint32 pos;
  uint32 u;
...
int cdb_find(struct cdb *c,const char *key,long long len)
{
  cdb_findstart(c);
